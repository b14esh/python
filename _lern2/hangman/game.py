...
from python.more2.hangman.invalid_operations_exeption import InvalidOperationException
...
class Game:
    ...
    def generate_world(self) -> str:
        ...
        self.__world = words[rand_index]

        self.__open_indexes = [False for _ in self.__world]
    ...
    def guess_letter(self, letter: str) -> Iterable[str]:
        if self.tries_counter == self.allowed_misses:
            raise InvalidOperationException(f"Exeeded the max misses number. Allowed {self.allowed_misses}")
